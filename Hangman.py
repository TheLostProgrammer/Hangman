...
title = Label(window, text="Enter a word for your opponent to guess:", font=("Helvetica", 14, "bold", "italic"))
title.grid(row=0, column=1)
wordInput = Entry(window, width=50)
wordInput.grid(row=1, column=1)

letters_title = Label(window, text="Wrong Letters", font=("Helvetica", 20, "bold", "underline"), foreground="grey")
...
def appearance():
    ...
    if len(letter) == 1:
        ...
        letters.config(text="\n".join([result[i:i + 26].upper() for i in range(0, len(result), 26)]), font=("Helvetica", 20))
        letters_title.config(text=f"Wrong Letters ({len(result)})", foreground="grey")
        ...
        if "".join(reveal) == word:
            ...
            panel.image = img
            panel.grid(row=5, column=1)
            word_display.config(text="You guessed the correct word. Game Over!\nPress \"New Game\" to play again.", font=("Helvetica", 20))
        if count > 13:
            count = -1
            letters_title.config(text="")
            used_letters.clear()
            ...
            panel.image = img
            panel.grid(row=5, column=1)
            word_display.config(text=f"You ran out of guesses. The word was \"{word}\"\nPress \"New Game\" to play again.",
                                font=("Helvetica", 20))
...
getwordButton = Button(window, text="Enter", command=get_word, width=10).grid(row=2, column=1)
quitButton = Button(window, text="Quit", width=buttonWidth, command=close).grid(row=1, column=2)
newgameButton = Button(window, text="New Game", width=buttonWidth, command=newgame).grid(row=1, column=0)
appearanceButton = Button(window, text="Enter", command=appearance, width=10).grid(row=9, column=1)
